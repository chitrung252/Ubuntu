...
$('a.thumbnail').on('click', function(e) {
  ...
  tol = ($(this).attr('href').lastIndexOf('/'))+1;
  ig = $(this).attr('href').length;
  
  fajlnev = '<?php if ($directory) echo urldecode($directory)."/"; ?>'+$(this).attr('href').substring(tol,ig);
  
  $('input[name=\'filename\']').attr('value', fajlnev);
	$('input[name=\'mask\']').attr('value', $(this).attr('href').substring(tol,ig));
...
$('#button-search').on('click', function() {
	var url = 'index.php?route=common/download_filemanager&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>';
...
$('#button-upload').on('click', function() {
	...
	$('#form-upload input[name=\'file\']').on('change', function() {
		$.ajax({
			url: 'index.php?route=common/download_filemanager/upload&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>',
			type: 'post',		
			dataType: 'json',
...
$('#button-folder').on('shown.bs.popover', function() {
	$('#button-create').on('click', function() {
		$.ajax({
			url: 'index.php?route=common/download_filemanager/folder&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>',
			type: 'post',		
			dataType: 'json',
...
$('#button-edit').on('shown.bs.popover', function() {
	$('#button-rename').on('click', function() {
      ...
      old = fajlok[0]['value'].substring(tol,ig);
   		...
   		$.ajax({
			url: 'index.php?route=common/download_filemanager/rename&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>',
			type: 'post',		
			dataType: 'json',
...
$('#modal-image #button-delete').on('click', function(e) {
  ...
  if (confirm('<?php echo $text_confirm; ?>')) {
		...
		$.ajax({
			url: 'index.php?route=common/download_filemanager/delete&token=<?php echo $token; ?>',
			type: 'post',		
			dataType: 'json',
